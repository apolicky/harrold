...
1. Finds the POST request to the Keycloak broker endpoint (`/broker/*/endpoint`)
2. Extracts the `SAMLResponse` parameter from the request body
3. Decodes and parses the SAML XML
4. Displays the role claims and all other attributes in a readable format
5. Shows the full formatted XML for inspection

No data leaves the browser — everything is processed locally.

## Usage

Open the app and drop a HAR file onto the upload area (or click to browse). The tool will find the SAML broker call automatically.

If the HAR contains multiple calls to the broker endpoint, you will be prompted to pick one.
